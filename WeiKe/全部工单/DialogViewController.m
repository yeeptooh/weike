...
- (void)sendButtonClicked:(UIButton *)sender {
    ...
    self.fromUserID = [NSString stringWithFormat:@"%@", @(userModel.ID)];
...
//    NSString *content = [self.textView.text stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSString *url = [NSString stringWithFormat:@"%@/Task.ashx?action=feedbackadd&taskid=%@&touserid=0&tousername=%@&fromuserid=%@&fromusername=%@&content=%@",HomeUrl,self.taskID,toName,self.fromUserID,fromName,content];
