...
- (void)setView {
    ...
    self.textfield.placeholder = @" 修改型号信息";
...
- (void)ensureButtonClicked {
    ...
    /*
     ...
     NSString *urlString = [NSString stringWithFormat:@"%@/Task.ashx?action=upBuyerName",HomeUrl];
     NSDictionary *params = @{@"id":@(self.ID),@"name":str,@"userid":@(model.ID),@"handler":model.Name};
     [AFNetClass POST_AFNetworkingRequestWithURL:urlString andParmas:params withReturnBlock:^(NSDictionary *response, NSError *error) {
     [MyProgressView dissmissWithError:@"修改成功"];
     NSLog(@"success");
     }];
     */
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    ...
    UserModel *model = [UserModel readUserModel];
    NSString *urlString = [NSString stringWithFormat:@"%@/Task.ashx?action=upProductType",HomeUrl];
    
    NSDictionary *params = @{@"id":@(self.ID),@"type":self.textfield.text ,@"userid":@(model.ID),@"handler":model.Name};
    ...
    [manager POST:urlString parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
        [self dismissViewControllerAnimated:YES completion:nil];
        self.returnType(self.textfield.text);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
